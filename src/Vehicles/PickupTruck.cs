...
/// <summary>
/// Pickup Truck - Heavy, 4WD, high torque.
/// Excellent for climbing, poor top speed.
/// </summary>
[GlobalClass]
public partial class PickupTruck : VehicleBase
{
    public PickupTruck()
    {
        Name = "Pickup Truck";
    ...
    protected override void InitializeVisuals()
    {
        ...
        {
            _bodySprite.Modulate = new Color(0.4f, 0.3f, 0.2f); // Brown
            _bodySprite.Texture = CreatePlaceholderTexture(new Vector2(140, 70), new Color(0.4f, 0.3f, 0.2f));
        }
        
        if (_frontWheelSprite != null && _frontWheelSprite.Texture == null)
        {
            _frontWheelSprite.Texture = CreateWheelTexture(0.4f, new Color(0.1f, 0.1f, 0.1f));
        }
        if (_rearWheelSprite != null && _rearWheelSprite.Texture == null)
        {
            _rearWheelSprite.Texture = CreateWheelTexture(0.4f, new Color(0.1f, 0.1f, 0.1f));
        }
    }
    ...
    private Texture2D CreateWheelTexture(float radiusMeters, Color color)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                {
                    image.SetPixel(x, y, new Color(0.4f, 0.35f, 0.3f));
                }
            }
        }
