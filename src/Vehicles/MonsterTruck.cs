...
/// <summary>
/// Monster Truck - Huge wheels, 4WD, high suspension.
/// Best for rough terrain and big jumps.
/// </summary>
[GlobalClass]
public partial class MonsterTruck : VehicleBase
{
    public MonsterTruck()
    {
        Name = "Monster Truck";
    ...
    protected override void InitializeVisuals()
    {
        ...
        {
            _bodySprite.Modulate = new Color(0.1f, 0.1f, 0.6f); // Blue
            _bodySprite.Texture = CreatePlaceholderTexture(new Vector2(130, 80), new Color(0.1f, 0.1f, 0.6f));
        }
        
        if (_frontWheelSprite != null && _frontWheelSprite.Texture == null)
        {
            _frontWheelSprite.Texture = CreateWheelTexture(0.6f, new Color(0.1f, 0.1f, 0.1f));
        }
        if (_rearWheelSprite != null && _rearWheelSprite.Texture == null)
        {
            _rearWheelSprite.Texture = CreateWheelTexture(0.6f, new Color(0.1f, 0.1f, 0.1f));
        }
    }
    ...
    private Texture2D CreateWheelTexture(float radiusMeters, Color color)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                if (dist <= radius && dist >= radius - 15)
                {
                    image.SetPixel(x, y, color);
                }
                else if (dist < radius - 15)
                {
                    image.SetPixel(x, y, new Color(0.2f, 0.2f, 0.3f));
                }
            }
        }
